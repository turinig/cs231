...
public:
	...
	// Const iterator for this list container.
	struct iterator {

		private:

			const struct node<T1>* node_curr_ptr; // ...

		public:

			// Constructors.
			iterator() { this->node_curr_ptr = nullptr; };
			iterator(const struct node<T1>* node) { this->node_curr_ptr = node; };

			// Operator "++" (prefix, chainable).
			struct iterator& operator ++() {
				std::cout << "A";
				if(this->node_curr_ptr != nullptr) { this->node_curr_ptr = this->node_curr_ptr->next; }
				return *this;
			}; // End operator "++" (prefix).
			...
			struct iterator operator ++(int) {
				std::cout << "B";
				iterator iter_temp = *this; // Temporary iterator copy (output).
				++*this; // Increment iterator (using prefix operator).
				return iter_temp; // Return iterator copy (pre-increment).
			}; // End operator "++" (postfix).

			// Operator "!=".
			bool operator !=(const iterator& other) const { return this->node_curr_ptr != other.node_curr_ptr; };

			// Operator "*" (dereference).
			T1 operator*() const { return this->node_curr_ptr->item; };

	};



	// Returing an iterator pointing to front of list.
	struct iterator begin() const { return iterator(this->head); };

	// Returning an iterator pointing to back of list (always null as marker for end of list).
	struct iterator end() const { return iterator(); };
